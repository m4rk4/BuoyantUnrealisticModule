...
def make_tweet(tweet_json, ref_tweet_url, is_parent=False, is_quoted=False, is_reply=0, has_parent=False):
    ...
    tweet_url = 'https://twitter.com/' + tweet_json['user']['screen_name'] + '/status/' + tweet_json['id_str']
    if has_parent:
        logo = ''
    else:
        ...
        logo = '<a href="' + ref_tweet_url + '"><img src="data:image/svg+xml;utf8,' + config.icon_x + '" style="width:100%;"/></a>'
    ...
    if is_parent or is_reply:
        avatar = config.server + '/image?url=' + quote_plus(tweet_json['user']['profile_image_url_https']) + '&width=48&height=48&mask=ellipse'
        if is_reply == 1:
            border = ''
        else:
