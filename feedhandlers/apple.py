...
def get_apple_playlist(url, args, save_debug=False):
  ...
  item['content_html'] = '<div><a href="{}"><img style="float:left; margin-right:8px;" src="{}"/></a><div>{}</a></div>'.format(api_data['attributes']['url'], poster, desc)
  ...
  for i, li in enumerate(list_items):
    if i == i_max:
      item['content_html'] += '</ol>&nbsp;&nbsp;&nbsp;&nbsp;<a href="{}/content?url={}">View all {}</a>'.format(config.server, quote_plus(item['url']), list_title.lower())
      break
  ...
  if api_data['type'] == 'playlists' or api_data['type'] == 'albums':
    if item['content_html'].endswith('</li>'):
