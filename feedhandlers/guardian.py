...
def get_content(url, args, site_json, save_debug=False):
    ...
    if '/video/' in url:
        ...
        if el:
            content_html += '<audio controls><source src="{0}" type="audio/mpeg">Your browser does not support the audio element.</audio><br /><a href="{0}"><small>Play audio</small></a>'.format(
                el.a['href'])
        el = soup.find(class_='content__standfirst')
        if el:
            if el.meta:
    ...
    if article_body:
        ...
        for el in article_body.find_all('figure', class_='element-atom'):
            if el['data-atom-type'] == 'media':
                ...
                if snippet_body:
                    ...
                    snippet_body[
                        'style'] = 'padding-left:1em; padding-right:1em; border:2px solid black; background-color:lightgrey;'
        ...
        for el in article_body.find_all('figure', class_='element-interactive'):
            if not 'sign-up' in str(el):
                new_el = BeautifulSoup('<iframe width="640px" height="480px" frameBorder="0" src={}></iframe'.format(
                    el['data-canonical-url']), 'html.parser')
                el.insert_after(new_el)
            el.decompose()
