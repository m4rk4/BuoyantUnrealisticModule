...
def get_item(post_json, args, save_debug):
    item = {}
    ...
    for el in soup.find_all(class_=['kg-card', 'kg-card-begin']):
        ...
        if 'kg-image-card' in el['class'] or ('kg-card-begin' in el['class'] and el.next_element.name == 'figure'):
            ...
            new_html = '<table style="width:100%; border-collapse:collapse;"><tr style="background:rgba(124,139,154,.13);">'
            it = el.find(class_='kg-callout-emoji')
            if it:
                new_html += '<td style="vertical-align:top; padding:1em; width:2em;">{}</td>'.format(it.get_text())
            it = el.find(class_='kg-callout-text')
            if it:
                new_html += '<td style="padding:1em 1em 1em 0;">{}</td>'.format(it.decode_contents())
            new_html += '</tr></td>'
