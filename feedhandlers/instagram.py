...
def get_content(url, args, save_debug=False):
  ...
  if media_type == 'GraphImage':
    for el in soup.find_all('img', class_='EmbeddedMediaImage'):
      if el.has_attr('srcset'):
        ...
        img_src = el['src']
      #post_media += utils.add_image('{}/image?url={}&width=480'.format(config.server, quote_plus(img_src)), link=img_src, style='float:left; display:inline;')
      post_media += '<a href="{}"><img style="float:left; display:inline" src="{}/image?url={}&width=480" /></a><br/>'.format(img_src, config.server, quote_plus(img_src))
    ...
    if ig_data:
      ...
      if el:
        ...
        #post_media += utils.add_image(poster, caption, link=ig_url, style='float:left; display:inline;')
        post_media += '<a href="{}"><img style="float:left; display:inline" src="{}" /><br/>{}<br/>'.format(ig_url, poster, caption)
  ...
  item['content_html'] = '<table style="max-width:500px; margin-left:auto; margin-right:auto; padding:10px; border:1px solid black; border-radius:10px; font-family:Roboto,Helvetica,Arial,sans-serif;"><tr><td style="width:50px;"><img style="vertical-align: middle;" src="{0}" /></td><td><a href="https://www.instagram.com/{1}"><b>{1}</b></a></td></tr><tr><td colspan="2" style="text-align:center;">'.format(avatar, username)
