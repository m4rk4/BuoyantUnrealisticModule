...
def resize_image(img_src, width=1000):
    ...
    return 'https://www.ft.com/__origami/service/image/v2/images/raw/{}?source=google-amp&fit=scale-down&width={}'.format(
        quote_plus(img_src), width)
...
def get_content(url, args, save_debug=False):
    ...
    if not article:
        ...
        return item
    if save_debug:
        utils.write_file(article.prettify(), './debug/debug.html')
    del article['class']
    ...
    for el in article.find_all(class_='n-content-video--youtube'):
        ...
        if it:
            ...
            logger.warning('unhandled n-content-video--youtube container in ' + url)
    ...
    for el in article.find_all(class_='n-content-layout'):
        if el.find('a', href=re.compile(
                r'https:\/\/ep\.ft\.com\/newsletters\/subscribe\?||https:\/\/ep\.ft\.com\/newsletters/[^\/]+\/subscribe')) or el.find(
                id=re.compile(r'recommended-newsletters-for-you')):
            el.decompose()
