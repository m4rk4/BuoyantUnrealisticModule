...
def get_content(url, args, site_json, save_debug):
    ...
    soup = BeautifulSoup(markdown(md, extras=['break-on-newline', 'code-friendly', 'fenced-code-blocks', 'tables']),
                         'html.parser')
        ...
        if m:
            ...
            if gist_js:
                ...
                if m:
                    ...
                    if gist:
                        new_html = '<h4>Gist: <a href="https://gist.github.com/{0}/{1}">https://gist.github.com/{0}/{1}</a></h4><pre style="margin-left:2em; padding:0.5em; white-space:pre; overflow-x:auto; background:#F2F2F2;">{2}</pre>'.format(
                            user, gist_id, html.escape(gist))
                        el.insert_after(BeautifulSoup(new_html, 'html.parser'))
                        el.decompose()
    ...
    for el in soup.find_all(class_='not-paragraph'):
        if el.find(class_='slogging'):
            ...
            if desc:
                if user.a:
                    new_html = '<div><a href="{}"><img style="float:left; margin-right:8px;" src="{}"/></a><div>{}</div><div style="clear:left;"></div>'.format(
                        user.a['href'], el.img['src'], desc)
                else:
                    new_html = '<div><img style="float:left; margin-right:8px;" src="{}"/><div>{}</div><div style="clear:left;"></div>'.format(
                        el.img['src'], desc)
                el.insert_after(BeautifulSoup(new_html, 'html.parser'))
            el.decompose()
...
def get_feed(url, args, site_json, save_debug):
    ...
    if m:
        ...
        query_url = 'https://{}-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20JavaScript%20(4.1.0)%3B%20Browser%20(lite)%3B%20JS%20Helper%20(3.5.5)%3B%20react%20(17.0.2)%3B%20react-instantsearch%20(6.12.1)&x-algolia-api-key={}&x-algolia-application-id={}'.format(
            sites_json['hackernoon']['x-algolia-application-id'], sites_json['hackernoon']['x-algolia-api-key'],
            sites_json['hackernoon']['x-algolia-application-id'].upper())
        query = '{{"requests":[{{"indexName":"stories","params":"highlightPreTag=%3Cais-highlight-0000000000%3E&highlightPostTag=%3C%2Fais-highlight-0000000000%3E&query=&maxValuesPerFacet=10&hitsPerPage=11&clickAnalytics=true&page=0&facets=%5B%22tags%22%5D&tagFilters=&facetFilters=%5B%22tags%3A{}%22%5D"}}]}}'.format(
            m.group(1))
