...
def resize_image(image, width=1000):
    return 'https://cdn.onebauer.media/{}/{}?format=jpg&quality=80&width={}&ratio=16-9&resize=aspectfill'.format(image['path'], image['fileName'], width)
...
def get_content(url, args, save_debug=False):
    ...
    if article_json.get('canonical'):
        ...
        item['url'] = re.sub(r'^{}'.format(article_json['publications'][0]['furl']), article_json['publications'][0]['hostname']['primary'], article_json['urls'][0])
    ...
    for layout in article_json['_layout']:
        if layout['type'] == 'content':
            ...
            content_html += markdown(layout['content']['text'].replace('{:target=_blank}', ''))
            ...
            content_html += utils.add_pullquote(layout['content']['text'])
            ...
            for image in layout['content']['images']:
                content_html += '<h4>{}</h4>'.format(image['titleText'])
                content_html += add_image(image)
                if image['description'].endswith('<br>'):
                    desc = image['description'][:-4]
                else:
                    desc = image['description']
                content_html += '<p>{}<br/><br/>£{} from <a href="{}">{}</a></p><hr/>'.format(desc, image['price'], image['actionLink'], urlsplit(image['actionLink']).netloc)
