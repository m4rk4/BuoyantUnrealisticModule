...
def process_content_element(element, url, func_resize_image, gallery=None):
    ...
    if element['type'] == 'text' or element['type'] == 'paragraph':
        ...
        if not re.search(r'amzn\.to|fanatics\.com|joinsubtext\.com|lids\.com|nflshop\.com', element['content'],
                         flags=re.I):
        ...
        if element['subtype'] == 'custom-audio':
            ...
            logger.warning('unhandled custom_embed in ' + url)

    elif element['type'] == 'divider':
        element_html += '<hr />'

    elif element['type'] == 'correction':
        element_html += '<blockquote><b>{}</b><br>{}</blockquote>'.format(element['correction_type'].upper(),
                                                                          element['text'])
...
def get_content_html(content, lead_image, func_resize_image, url, save_debug=False):
    ...
    if not (content['content_elements'][0]['type'] == 'image' or (
            content['content_elements'][0]['type'] == 'oembed_response' and content['content_elements'][0][
        'subtype'] == 'youtube')):
        if content.get('multimedia_main'):
            if content['multimedia_main']['type'] == 'gallery':
                content_html += process_content_element(content['multimedia_main']['content_elements'][0], url,
                                                        func_resize_image)
            else:
    ...
    if content.get('related_content') and content['related_content'].get('videos'):
        ...
        for video in content['related_content']['videos']:
            ...
            content_html += utils.add_video(video['source']['mp4'], 'video/mp4',
                                            video['thumbnail']['renditions']['original']['480w'], caption)
    return content_html
