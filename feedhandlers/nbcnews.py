...
def get_content(url, args, save_debug=False):
  ...
  if article_json.get('date'):
    ...
    dt_pub = datetime.strptime(article_json['datePublished'], '%a %b %d %Y %H:%M:%S GMT+0000 (UTC)').astimezone(timezone.utc)
    dt_mod = datetime.strptime(article_json['dateModified'], '%a %b %d %Y %H:%M:%S GMT+0000 (UTC)').astimezone(timezone.utc)
      ...
      if el['type'] == 'markup':
        ...
        if el['widget']['name'] == 'CUSTOM_EMBED':
          if el['widget']['properties']['embed']['type'] == 'BLOCKQUOTE':
            ...
            widget_html = utils.add_pullquote(el['widget']['properties']['embed']['text'].strip(), el['widget']['properties']['embed']['attribution'])
          ...
          if embed_soup.div and embed_soup.div.has_attr('class'):
            if 'iframely-twitter' in embed_soup.div['class']:
              ...
              widget_html = '<blockquote><b>Embedded content from <a href="{0}">{0}</a></b></blockquote>'.format(el['widget']['fallbackUrl'])
          ...
          if m:
            widget_html = '<blockquote><b>Embedded content from <a href="{0}">{0}</a></b></blockquote>'.format(m.group(0))
        ...
        poster = '{}/image?height=128&url={}'.format(config.server, el['product']['teaseImage']['url']['primary'])
        desc = '<h4 style="margin-top:0; margin-bottom:0.5em;"><a href="{}">{}</a></h4>${} at <a href="{}">{}</a>'.format(utils.get_redirect_url(el['product']['offers'][0]['externalUrl']), el['product']['name'], el['product']['offers'][0]['price'], el['product']['offers'][0]['seller']['externalUrl'], el['product']['offers'][0]['seller']['name'])
        item['content_html'] += '<div><img style="float:left; margin-right:8px;" src="{}"/></a><div>{}</div><div style="clear:left;">&nbsp;</div>'.format(poster, desc)
  ...
  for layout in next_json['props']['initialState']['front']['curation']['layouts']:
    for package in layout['packages']:
      for item in package['items']:
        ...
        dt = datetime.strptime(item['item']['datePublished'], '%a %b %d %Y %H:%M:%S GMT+0000 (UTC)').astimezone(timezone.utc)
