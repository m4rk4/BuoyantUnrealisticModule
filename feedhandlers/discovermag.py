...
def get_next_json(url):
  ...
  if not next_json:
    ...
    if m:
      ...
      next_json = utils.get_url_json('https://www.discovermagazine.com/_next/data/{}{}.json'.format(m.group(1), split_url.path))
      if not next_json:
        return None
  return next_json
...
def get_content(url, args, save_debug):
  ...
  def process_content(content):
    ...
    if content['nodeType'] == 'paragraph':
      ...
      if content['data']['target']['fields'].get('file') and 'image' in content['data']['target']['fields']['file']['contentType']:
        if content['data']['target']['fields'].get('description'):
          caption = content['data']['target']['fields']['description']
        else:
          caption = ''
        content_html = utils.add_image(resize_image(content['data']['target']['fields']['file']['url']), caption)
      else:
