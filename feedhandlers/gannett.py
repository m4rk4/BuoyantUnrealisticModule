...
def get_content(url, args, save_debug=False):
    ...
    if el:
        ...
        if el:
            ...
            m = re.search(
                r'Published:?\s(\d{1,2}):(\d{2})\s([ap])\.m\.\s([A-Z]{2,3})\s([a-zA-Z]{3})\.\s(\d{1,2}),\s(202\d)',
                pub_time)
            if m:
                dt = None
                dt_str = '{}:{} {}M {} {} {}'.format(m.group(1).zfill(2), m.group(2), m.group(3).upper(), m.group(5),
                                                     m.group(6).zfill(2), m.group(7))
            ...
            m = re.search(
                r'Updated:?\s(\d{1,2}):(\d{2})\s([ap])\.m\.\s([A-Z]{2,3})\s([a-zA-Z]{3})\.\s(\d{1,2}),\s(202\d)',
                pub_time)
            if m:
                dt = None
                dt_str = '{}:{} {}M {} {} {}'.format(m.group(1).zfill(2), m.group(2), m.group(3).upper(), m.group(5),
                ...
                if video_json:
                    ...
                    if video:
                        new_el = BeautifulSoup(
                            utils.add_video(video['_video'], 'video/mp4', video['_image'], video['summary']),
                            'html.parser')
