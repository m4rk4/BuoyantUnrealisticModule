...
def format_block(block_id, initial_state, arg1=None, arg2=None):
  ...
  if block['__typename'] == 'Dropzone' or block['__typename'] == 'RelatedLinksBlock' or block[
    ...
    '__typename'] == 'HeaderMultimediaBlock' or block['__typename'] == 'HeaderFullBleedVerticalBlock' or block['__typename'] == 'HeaderFullBleedHorizontalBlock':
    ...
    for content in block['content@filterEmpty']:
      block_html += format_block(content['id'], initial_state)
      ...
      if embed_json.get('slides'):
        for slide in embed_json['slides']:
          if slide['__typename'] == 'Image':
            ...
            logger.warning('unhandled EmbeddedInteractive slide type {} in {}'.format(slide['__typename'], block_id))
    if embed_html:
      ...
      block_html += '<h4><a href="data:text/html;base64,{}">Embedded content</a></h4>'.format(embed_b64.decode('utf-8'))
    ...
    if block.get('hubAssets'):
      block_html += '<h3><a href="{}">{}</a></h3>'.format(format_block(block['hubAssets'][0]['id'], initial_state), block['displayName'])
          ...
          if data_data['type'] == 'lede':
            for data_data_data in data_data['data']:
              if data_data_data['type'] == 'text':
                ...
                logger.debug('unhandled experimentalJsonBlob data type {}'.format(data_data_data['type']))
...
def get_block_item(block_id, initial_state):
  ...
  if dt:
    ...
    item['_display_date'] = '{}. {}, {}, {}:{} {}'.format(dt_est.strftime('%b'), dt_est.day, dt_est.year, int(dt_est.strftime('%I')), dt_est.strftime('%M'), dt_est.strftime('%p').lower())
    #item['_display_date'] = '{}. {}, {}'.format(dt.strftime('%b'), dt.day, dt.year)
