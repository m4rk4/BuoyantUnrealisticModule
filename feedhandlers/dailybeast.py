...
def resize_image(image, width=1000, crop_square=False):
  ...
  if crop_square:
    if h < w:
      ...
      y = float(h - w)/2.0
      h = w
  img_src = 'https://img.thedailybeast.com/image/upload/c_crop,d_placeholder_euli9k,h_{},w_{},x_{},y_{}/dpr_1.5/c_limit,w_{}/fl_lossy,q_auto'.format(math.ceil(h), math.ceil(w), math.ceil(x), math.ceil(y), math.ceil(width/1.5))
          ...
          if conv_card['expanded']:
            ...
            if conv_card.get('image'):
              ...
              for offer in conv_card['partnerButtons']:
                if offers:
                  offers += '<br/>'
                offers += '&bull;&nbsp;<a href="{}">Buy at {}</a>'.format(utils.get_redirect_url(offer['partnerUrl']), offer['partnerName'])
                if offer.get('price'):
                  offers += ': ${}'.format(offer['price'])
              card_html = '<div><img style="height:128px; float:left; margin-right:8px;" src="{}"/><div><h4 style="margin-top:0; margin-bottom:0.5em;">{}</h4>{}</div><div style="clear:left;"></div>'.format(img_src, conv_card['promoItemName'], offers)
            else:
              card_html = '<h4>{}</h4><ul>'.format(conv_card['promoItemName'])
              for offer in conv_card['partnerButtons']:
                card_html += '<li><a href="{}">Buy at {}</a>'.format(utils.get_redirect_url(offer['partnerUrl']), offer['partnerName'])
                if offer.get('price'):
                  card_html += ': ${}'.format(offer['price'])
                card_html += '</li>'
              card_html += '</ul>'
...
def get_content(url, args, save_debug=False):
  ...
  if article_json.get('read_it_at') and article_json['read_it_at'].get('url'):
    item['content_html'] += '<p>Read it at <a href="{}">{}</a></p>'.format(article_json['read_it_at']['url'], article_json['read_it_at']['name'])
